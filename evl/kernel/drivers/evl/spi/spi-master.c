			       ...
			       struct evl_spi_config *config)
{
	...
	evl_lock_kmutex(&master->bus_lock);
	...
	if (ret) {
		...
		evl_unlock_kmutex(&master->bus_lock);
		return ret;
	}
...
int __evl_spi_setup_driver(struct evl_spi_master *master)
{
	...
	evl_spin_lock_init(&master->lock);
	evl_init_kmutex(&master->bus_lock);
...
void evl_spi_remove_master(struct evl_spi_master *master)
{
	kfree(master->tx_buf);
	kfree(master->rx_buf);
	evl_destroy_kmutex(&master->bus_lock);
