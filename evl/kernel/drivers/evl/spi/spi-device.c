...
static ssize_t
spidev_oob_read(struct file *filp, char __user *buf,
				size_t count)
{
...
static ssize_t
spidev_oob_write(struct file *filp, const char __user *buf,
		size_t count)
{
	struct evl_spidevice *evl_spidev;
...
static int spidev_open(struct inode *inode, struct file *filp)
{
	...
	status = evl_open_file(&evl_spidev->efile, filp);
	if (status)
		 goto err_find_dev; 
	spidev->users++;
...
static int spidev_release(struct inode *inode, struct file *filp)
{
	...
	mutex_lock(&device_list_lock);
	evl_spidev = filp->private_data;
	evl_release_file(&evl_spidev->efile);
...
static const struct file_operations spidev_fops = {
	.owner =	THIS_MODULE,
	.oob_write =	spidev_oob_write,
	.oob_read =		spidev_oob_read,
			      ...
			      struct spi_device *spi)
{
	...
	evl_spin_lock_irqsave(&master->lock, flags);
	list_add_tail(&slave->next, &master->slaves);
	evl_spin_unlock_irqrestore(&master->lock, flags);
