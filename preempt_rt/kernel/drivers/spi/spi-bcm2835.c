...
static irqreturn_t bcm2835_spi_interrupt(int irq, void *dev_id)
{
	...
	if (!bs->rx_len) {
		...
		bs->interrupt_done = 1;
		smp_mb();
		if (swait_active(&bs->swait))
			swake_up_one(&bs->swait);
#endif
	}
					...
					u32 cs)
{
	...
	bs->interrupt_done = 0;
	for (;;) {
		prepare_to_swait_exclusive(&bs->swait, &swait, TASK_INTERRUPTIBLE);
		/* smp_mb() from set_current_state() */
	 	if (bs->interrupt_done)
 			break;
 		schedule();
 	}
	finish_swait(&bs->swait, &swait);
...
static int bcm2835_spi_probe(struct platform_device *pdev)
{
	...
	init_swait_queue_head(&bs->swait);
