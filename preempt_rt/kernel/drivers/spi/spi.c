...
{									\
	unsigned long flags;						\
	ssize_t len;							\
	raw_spin_lock_irqsave(&stat->lock, flags);				\
	len = sprintf(buf, format_string, stat->field);			\
	raw_spin_unlock_irqrestore(&stat->lock, flags);			\
	return len;							\
}									\
				       ...
				       struct spi_controller *ctlr)
{
	...
	raw_spin_lock_irqsave(&stats->lock, flags);
	...
	raw_spin_unlock_irqrestore(&stats->lock, flags);
}
...
struct spi_device *spi_alloc_device(struct spi_controller *ctlr)
{
	...
	raw_spin_lock_init(&spi->statistics.lock);
...
static void __spi_pump_messages(struct spi_controller *ctlr, bool in_kthread)
{
	...
	/* Lock queue */
	raw_spin_lock_irqsave(&ctlr->queue_lock, flags);

	/* Make sure we are not already running a message */
	if (ctlr->cur_msg) {
		raw_spin_unlock_irqrestore(&ctlr->queue_lock, flags);
		return;
	}
...
{
	/* set priority with 10 greater than threaded interrupts priority */
	struct sched_param param = { .sched_priority = 10 + MAX_RT_PRIO / 2 };
...
int spi_register_controller(struct spi_controller *ctlr)
{
	...
	raw_spin_lock_init(&ctlr->queue_lock);
	raw_spin_lock_init(&ctlr->bus_lock_spinlock);
	...
	/* add statistics */
	raw_spin_lock_init(&ctlr->statistics.lock);
...
int spi_async(struct spi_device *spi, struct spi_message *message)
{
	...
	raw_spin_lock_irqsave(&ctlr->bus_lock_spinlock, flags);
	...
	raw_spin_unlock_irqrestore(&ctlr->bus_lock_spinlock, flags);
...
{
	struct spi_message *message = (struct spi_message*)arg;
 	/* using swait.h model */
	message->message_transfered = 1;
	smp_mb();
	if (swait_active(&message->swait))
		swake_up_one(&message->swait);
}
...
static int __spi_sync(struct spi_device *spi, struct spi_message *message)
{
	...
	if (status == 0) {
		...
		/* wait for transfer completion using swait.h model */
		for (;;) {
			prepare_to_swait_exclusive(&message->swait, &swait, TASK_INTERRUPTIBLE);
			/* smp_mb() from set_current_state() */
			if (message->message_transfered)
				break;
			schedule();
		}
		finish_swait(&message->swait, &swait);
