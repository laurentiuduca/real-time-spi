...
int open_spi_device(char* spi_dev_name) 
{
	...
	/* Configure device */
	iobufs.io_len = LAT_SPI_BUFFER_SIZE;
	...
	printf("input_area[%u..%u], output_area[%u..%u], mapping length=%u \n",
			 iobufs.i_offset, iobufs.i_offset + LAT_SPI_BUFFER_SIZE - 1,
			 iobufs.o_offset, iobufs.o_offset + LAT_SPI_BUFFER_SIZE - 1,
			 iobufs.map_len);
	...
	config.speed_hz = LAT_SPI_SPEED_HZ;
