...
to_slave_bcm2835(struct evl_spi_remote_slave *slave)
{
	return container_of(slave, struct spi_slave_bcm2835, slave);
}

static inline struct spi_master_bcm2835 *
to_master_bcm2835(struct evl_spi_remote_slave *slave)
{
...
static irqreturn_t bcm2835_spi_interrupt(int irq, void *data)
{
	struct spi_master_bcm2835 *spim;

	spim = (struct spi_master_bcm2835*) data;
...
static int bcm2835_configure(struct evl_spi_remote_slave *slave)
{
...
static void bcm2835_chip_select(struct evl_spi_remote_slave *slave,
				bool active)
{
	struct spi_master_bcm2835 *spim = to_master_bcm2835(slave);
	struct evl_spi_config *config = &slave->config;
...
static int bcm2835_transfer_iobufs(struct evl_spi_remote_slave *slave)
{
	struct spi_master_bcm2835 *spim = to_master_bcm2835(slave);
...
static int bcm2835_transfer_iobufs_n(struct evl_spi_remote_slave *slave,
										 int len)
{
	struct spi_master_bcm2835 *spim = to_master_bcm2835(slave);
	struct spi_slave_bcm2835 *bcm = to_slave_bcm2835(slave);
...
static ssize_t bcm2835_read(struct evl_spi_remote_slave *slave,
			    void *rx, size_t len)
{
	struct spi_master_bcm2835 *spim = to_master_bcm2835(slave);
...
static ssize_t bcm2835_write(struct evl_spi_remote_slave *slave,
...
static int bcm2835_set_iobufs(struct evl_spi_remote_slave *slave,
...
static int bcm2835_mmap_iobufs(struct evl_spi_remote_slave *slave,
			       struct vm_area_struct *vma)
{
	...
	/* map bcm->io_virt to vma->vm_start */
	pfn = __pa(bcm->io_virt) >> PAGE_SHIFT;
	return remap_pfn_range(vma, vma->vm_start, pfn, size, PAGE_SHARED);
}

static void bcm2835_mmap_release(struct evl_spi_remote_slave *slave)
{
	struct spi_slave_bcm2835 *bcm = to_slave_bcm2835(slave);
...
bcm2835_attach_slave(struct evl_spi_master *master, struct spi_device *spi, struct device *dev)
{
	...
	ret = evl_spi_add_remote_slave(&bcm->slave, master, spi, dev);
	if (ret) {
		dev_err(&spi->dev,
...
static void bcm2835_detach_slave(struct evl_spi_remote_slave *slave)
{
	struct spi_slave_bcm2835 *bcm = to_slave_bcm2835(slave);
...
static struct evl_spi_master_ops bcm2835_master_ops = {
...
static int bcm2835_spi_probe(struct platform_device *pdev)
{
	...
	master->subclass = evl_SUBCLASS_BCM2835;
	...
	ret = devm_request_irq(&pdev->dev, spim->irq,
				  bcm2835_spi_interrupt, IRQF_NO_THREAD, pdev->name,
				  spim);
	if (ret) {
